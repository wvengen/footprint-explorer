...
function CircleGrid({data, r, sf, x, y, ird=1, children}) {
  const entries = Object.entries(data).reduce((r, [k,v]) => r.concat(Array(v).fill(k)), []);
  const eld = 2*r / (ird+2); // diameter of element including margin
  let els = [];

  if (entries.length == 1) {
    // @todo make generic solution for 'in-between' layouts
    els.push(<SvgIcon key={[0,0]} id={entries[0]} x={0} y={0} size={eld/sf} />);
  } else {
    for (let iy=-ird/2; iy<=ird/2 && els.length < entries.length; iy++) {
      for (let ix=-ird/2; ix<=ird/2 && els.length < entries.length; ix++) {
        if (((ix*ix) + (iy*iy)) <= (ird*ird/4)) {
          const cx = ix * eld;
          const cy = iy * eld;
          els.push(<SvgIcon key={[ix,iy]} id={entries[els.length]} x={cx} y={cy} size={eld/sf} />);
        }
      }
    }
  }
  if (entries.length <= els.length) {
    return <g transform={"translate("+x+","+y+")"}>{els}</g>;
  } else {
    // hack to make sure circle fits
    // @todo add initial lower-bound guess
    return CircleGrid({data, r, sf, x, y, ird: ird+1});
  }
}
...
function VisualiserCloud({usages, impacts, sources}) {
  return (
    <svg style={styles.container}>
      ...
      <CircleGrid x={250} y={300} r={130} sf={0.85} data={usages} />
      <Spoon x={406} y={130} />
    </svg>
  );
}
